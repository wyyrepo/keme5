...
bool QDbfTablePrivate::open(QDbfTable::OpenMode openMode)
{
    ...
    for (int i = 0; i < fieldDescriptorsLength; i += FIELD_DESCRIPTOR_LENGTH) {
        ...
        quint8 fieldTypeChar = static_cast<quint8>(fddata[i + FIELD_NAME_LENGTH] & 0xFF);
        ...
        const int fieldLength = static_cast<int>(fddata[i + FIELD_LENGTH_OFFSET] & 0xFF);
        const int fieldPrecision = static_cast<int>(fddata[i + FIELD_PRECISION_OFFSET] & 0xFF);
