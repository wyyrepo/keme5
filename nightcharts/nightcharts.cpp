...
void Nightcharts::set_max_absolute(double qmax)
{
  maxvalue=qmax;
  if (maxvalue>-0.001 && maxvalue < 0.001) maxvalue=0.01;
...
int Nightcharts::addPiece(QString name,Qt::GlobalColor color, double abs_value)
{
    this->nPiece++;
    pieceNC piece;
    piece.addName(name);
    piece.setColor(color);
    piece.setPerc(abs_value/maxvalue*100);
    piece.setabsvalue(abs_value);
    pieces.append(piece);
    //absvalue=abs_value;
    return 0;
...
void Nightcharts::drawLegend(QPainter *painter)
{
    ...
    case Nightcharts::Vertical:
    {
        ...
        for (int i=pieces.size()-1;i>=0;i--)
        {
            ...
            QString label = pieces[i].pname + " - " + QString::number(pieces[i].pPerc)+"%";
            if (absolute_values) label.setNum(pieces[i].absvalue,'f',2);
            painter->drawText(x+painter->fontMetrics().height()+dist,y+painter->fontMetrics().height()/2+dist,pieces[i].pname + " - " + label);
        }
        break;
    }
...
void pieceNC::setabsvalue(double value)
{
  absvalue=value;
}
